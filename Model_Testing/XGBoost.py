...
# Display floating-point arrays (such as predicted probabilities) to four decimals.
...
#train = train.sample(frac=0.0005, random_state=42)
...
# BINARY-CLASSIFICATION ROLLBACK: uncomment these lines and comment out the
# multiclass Tracker block below when changing back to a binary dataset.
...
# BINARY-CLASSIFICATION ROLLBACK: these remain valid for the binary predictions above.
...
for class_index, class_label in enumerate(class_labels):
    ...
    plt.plot(pred_prob, true_prob, marker="o", label=f"XGBoost class {class_label}")
